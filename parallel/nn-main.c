...
void trainN()
{
    ...
    shared(ranpat, Error, DeltaWeightHO, DeltaWeightIH, stdout, \
    WeightIH, Hidden, WeightHO, Output, DeltaO, DeltaH, \
    alpha, tSet, Target, eta) \
    private(SumH, SumDOW, SumO, BError)
    {
        printf("Number of thread is %d\n", omp_get_thread_num());
        int continueLearning = 1;
        for (int epoch = 0; epoch < 1000000 && continueLearning == 1; epoch++)
        {

            // Faster with one thread (= Overhead of Parallelism)
            #pragma omp single
            {
            ...
            // See if single is better than for...
            // #pragma omp single
            // {
            #pragma omp for
                for (int p = 0; p < NUMPAT; p++)
